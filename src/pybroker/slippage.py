...
class VolatilityVolumeSlippageModel(SlippageModel):
    ...
    def __init__(self,
        ...
        self.metrics = {} # Store metrics per symbol

        # Prepare data: Ensure 'date' is the index
        ...
        if not has_required:
            raise ValueError("Input data must contain 'symbol', 'high', 'low', 'close' columns.")
        ...
        for symbol, group_df in data.groupby('symbol'):
            ...
            if has_volume and not group_df['volume'].isnull().all():
                ...
                symbol_metrics['avg_volume'] = vol_filled.rolling(
                    window=self.vol_window, min_periods=1
                ).mean().fillna(0)
            else:
                 symbol_metrics['avg_volume'] = Decimal('0')

            symbol_metrics['atr'] = symbol_metrics['atr'].apply(lambda x: Decimal(str(x)) if pd.notna(x) else Decimal('0'))
            symbol_metrics['avg_volume'] = symbol_metrics['avg_volume'].apply(lambda x: Decimal(str(x)) if pd.notna(x) else Decimal('0'))
    ...
    def apply_slippage(
        ...
        try:
            current_date = ctx.dt
            current_date = current_date.tz_localize(None) if current_date.tzinfo else current_date

        except AttributeError:
             print(f"Slippage Warning: ctx.dt not found for {symbol}. Cannot apply slippage.")
             if buy_shares: ctx.buy_shares = buy_shares
             if sell_shares: ctx.sell_shares = sell_shares
             return
        ...
        try:
            ...
            if symbol_metrics is None:
                print(f"Slippage Warning: No metrics pre-calculated for symbol {symbol}. Applying zero slippage.")
                if buy_shares: ctx.buy_shares = buy_shares
                if sell_shares: ctx.sell_shares = sell_shares
                return

            metrics_today = symbol_metrics.loc[:current_date].iloc[-1] if current_date >= symbol_metrics.index.min() else None

            if metrics_today is None or metrics_today.name != current_date:
                 print(f"Slippage Warning: Metrics not found for {symbol} on or before {current_date}. Applying zero slippage.")
                 if buy_shares: ctx.buy_shares = buy_shares
                 if sell_shares: ctx.sell_shares = sell_shares
                 return
            ...
            # Determine price: Use fill price if available (post-trade), else last close
            # Removed check for ctx.target_price as it doesn't exist on ExecContext
            if hasattr(ctx, 'fill_price') and ctx.fill_price:
                 ...
                 print(f"Slippage Warning: Cannot determine price for {symbol} on {current_date}. Applying zero slippage.")
                 if buy_shares: ctx.buy_shares = buy_shares
                 if sell_shares: ctx.sell_shares = sell_shares
                 return
            ...
            print(f"Slippage Warning: Metric key error for {symbol} on {current_date}: {e}. Applying zero slippage.")
            if buy_shares: ctx.buy_shares = buy_shares
            if sell_shares: ctx.sell_shares = sell_shares
        except Exception as e:
            import traceback
            print(f"Slippage Error for {symbol} on {current_date}: {e}\n{traceback.format_exc()}. Applying zero slippage.")
            if buy_shares: ctx.buy_shares = buy_shares
            if sell_shares: ctx.sell_shares = sell_shares
